...
export const productsApi = {
  ...
  getTrendingSearchTerms: async (): Promise<string[]> => {
    try {
      ...
      const productNames = response.documents.map((doc: any) => doc.name);
      const staticTerms = ['iPhone 15', 'سماعات بلوتوث', 'لابتوب', 'ساعة ذكية', 'ملابس رياضية'];

      return Array.from(new Set([...staticTerms, ...productNames])).slice(0, 8);
    } catch (error) {
      console.error("Error fetching trending searches:", error);
      return ['iPhone 15', 'سماعات بلوتوث', 'لابتوب', 'ساعة ذكية'];
    }
  },
