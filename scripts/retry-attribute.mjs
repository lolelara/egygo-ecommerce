
import { Client, Databases } from "node-appwrite";

const client = new Client()
    .setEndpoint("https://cloud.appwrite.io/v1")
    .setProject("68d8b9db00134c41e7c8")
    .setKey("standard_");

const databases = new Databases(client);
const DATABASE_ID = "68de037e003bd03c4d45";
const USER_PREFERENCES_COLLECTION_ID = "userPreferences";

async function retryAttribute() {
    try {
        console.log("Retrying isFeatured attribute...");
        try {
            await databases.createBooleanAttribute(DATABASE_ID, USER_PREFERENCES_COLLECTION_ID, "isFeatured", false, false);
            console.log("✅ Added attribute: isFeatured");
        } catch (error) {
            if (error.code === 409) {
                console.log("ℹ️ Attribute isFeatured already exists.");
            } else {
                console.error("❌ Error adding attribute isFeatured:", error.message);
            }
        }
    } catch (error) {
        console.error("Error:", error);
    }
}

retryAttribute();
