
import { Client, Databases } from 'node-appwrite';

// Configuration
const ENDPOINT = 'https://cloud.appwrite.io/v1';
const PROJECT_ID = '68d8b9db00134c41e7c8';
const API_KEY = 'standard_';
const DATABASE_ID = '68de037e003bd03c4d45';
const COLLECTION_ID = 'products';

const client = new Client()
    .setEndpoint(ENDPOINT)
    .setProject(PROJECT_ID)
    .setKey(API_KEY);

const databases = new Databases(client);

async function updateIndexes() {
    try {
        console.log('Adding fulltext index to name attribute in products collection...');
        // createIndex(databaseId, collectionId, key, type, attributes, orders)
        await databases.createIndex(
            DATABASE_ID,
            COLLECTION_ID,
            'name_search_index', // key
            'fulltext',          // type
            ['name'],            // attributes
            ['ASC']              // orders (optional for fulltext but required by SDK signature sometimes)
        );
        console.log('Success! Fulltext index added to name attribute.');
    } catch (error) {
        console.error('Error adding index:', error.message);
    }
}

updateIndexes();
