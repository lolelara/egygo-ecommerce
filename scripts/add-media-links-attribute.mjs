import { Client, Databases } from 'node-appwrite';

// Appwrite Configuration
const ENDPOINT = 'https://cloud.appwrite.io/v1';
const PROJECT_ID = '68d8b9db00134c41e7c8';
const API_KEY = 'standard_';
const DATABASE_ID = '68de037e003bd03c4d45';

const client = new Client()
    .setEndpoint(ENDPOINT)
    .setProject(PROJECT_ID)
    .setKey(API_KEY);

const databases = new Databases(client);

async function addMediaLinksAttribute() {
    console.log('🚀 Adding mediaLinks attribute to products collection...');

    try {
        await databases.createStringAttribute(
            DATABASE_ID,
            'products',
            'mediaLinks',
            1000,   // Size
            false,  // Required
            null,   // Default
            true    // Array
        );
        console.log('✅ mediaLinks attribute created successfully!');
    } catch (error) {
        if (error.message.includes('already exists')) {
            console.log('⚠️  mediaLinks attribute already exists.');
        } else {
            console.error('❌ Error creating attribute:', error.message);
        }
    }
}

addMediaLinksAttribute();
